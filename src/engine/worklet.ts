...
class KoeProcessor extends AudioWorkletProcessor {
  ...
  private dequeue(): ActiveNote | null {
    ...
    if (note.phoneme === '') {
      return {
        data: SILENT, length: 0, consonant: 0, pre: 0, overlap: 0,
        readPos: 0, stepRate: 1, remaining: note.duration,
      };
    }
  ...
  private readSample(data: Int16Array, idx: number): number {
    const i = idx | 0;
    const frac = idx - i;
    return ((data[i] ?? 0) + ((data[i + 1] ?? 0) - (data[i] ?? 0)) * frac) / 32768;
  }
  ...
  process(_inputs: Float32Array[][], outputs: Float32Array[][]): boolean {
    ...
    for (let i = 0; i < out.length; i++) {
      ...
      if (!this.current || this.current.remaining <= 0) {
        ...
        this.next = null;
        if (!this.current) { out[i] = 0; continue; }
      ...
      if (this.next) {
        ...
        if (cur.remaining <= lead) {
          const xfade = Math.min(Math.max(nxt.overlap / nxt.stepRate, MIN_CROSSFADE), lead);
