...
export const samplesToMs = (samples: number): number => (samples / WORLDLINE_SAMPLE_RATE) * 1000;
 ...
 */
export function leadInFromEntry(entry: PhonemeEntry): { preMs: number; consonantMs: number } {
...
function injectScript(src: string): Promise<void> {
  return new Promise((resolve, reject) => {
    const existing = document.querySelector<HTMLScriptElement>(`script[data-koe-worldline="${src}"]`);
    if (existing) {
      resolve();
      return;
    }
...
export class Worldline {
  ...
  renderNote(params: RenderNoteParams): Float32Array | null {
    ...
    const audio =
      outLen > 0 ? new Float32Array(WL.HEAPF32.buffer, yPtr, outLen).slice() : null;
    WL._free(yPtrPtr);
    WL._PhraseSynthDelete(ps);
    return audio;
  }
